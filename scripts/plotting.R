...
for (crop_group in unique(default_drift_reference_df$`Crop grouping`)){
  ...
  full_drift <- mean_drift_dep(params = get_params(crop_grouping = crop_group,num_apps = 1))
  ...
  varying_band_plot <-
    ...
    labs(
      title = "Percentage of drift compared to a full field application for various band and inter band widths",
      subtitle = paste0(crop_group," assuming 1m between edge of field and water body"),
      x = "Band Width (Spray) [m]",
      y = "Inter Band Width (No Spray) [m]",
      fill = "Percentage Reduction from \nFull Field Application"
    ) +
  ...
  ggsave(paste0("plots/crop_group/",crop_group,"band_vs_inter_band_width.png"), plot = varying_band_plot, width = 10, height = 8)
  ...
  ggsave(paste0("plots/crop_group/",crop_group,"band_vs_inter_band_width_zoom.png"), plot = varying_band_zoom_plot, width = 10, height = 8)
...
expand.grid(
  ...
  mutate(z_1=`distance from crop to top of bank (m)`+`distance from top of bank to water (m)`,
         z_2=z_1+`water width (m)`)

for (i in seq_len(nrow(focus_crop_combinations))){

  this_param <- get_params(crop_grouping = focus_crop_combinations$`Crop grouping`[i],
                           num_apps = 1)
  this_crop <- focus_crop_combinations$Crop[i]
  this_water_body <- focus_crop_combinations$`water body`[i]


  full_drift <- mean_drift_dep(z_1 = focus_crop_combinations$z_1[i],z_2 = focus_crop_combinations$z_2[i],params =this_param )
  ...
  varying_band_plot <-
    ...
    labs(
      title = "Percentage of drift compared to a full field application for various band and inter band widths",
      subtitle = paste0("Crop: ",this_crop,"\nWater body: FOCUS ",this_water_body),
      x = "Band Width (Spray) [m]",
      y = "Inter Band Width (No Spray) [m]",
      fill = "Percentage Reduction from \nFull Field Application",
    ) +
  ...
  varying_band_plot
  ggsave(paste0("plots/focus_crop_group/",this_crop,"_",this_water_body,".png"), plot = varying_band_plot, width = 10, height = 8)
  ...
  ggsave(paste0("plots/focus_crop_group/",this_crop,"_",this_water_body,"_zoom.png"), plot = varying_band_zoom_plot, width = 10, height = 8)
